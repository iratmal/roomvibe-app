          ...
          <div className="relative overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            <div className="flex items-center justify-between border-b border-slate-100 px-4 py-3 text-sm">
              ...
              <div className="text-slate-500">
                {userPhoto ? <span>Custom wall photo</span> : <>Wall: <span className="font-mono text-xs">{wall.toUpperCase()}</span></>}
              </div>
            </div>
            ...
            <div className="flex items-center justify-between border-t border-slate-100 px-4 py-2 text-xs text-slate-500">
              <div>Preview only · Size: {widthVal}×{heightVal} {sizeUnit}</div>
        ...
        <Modal onClose={() => setShowSizingInfo(false)}>
          <div className="text-sm text-slate-700">
            ...
            <ol className="mt-3 list-decimal space-y-2 pl-5">
              <li><b>Reference object:</b> Place a standard A4 paper (210 × 297 mm) or a credit card on the wall and take a photo.</li>
              <li><b>Calibrate:</b> Mark the reference corners in the photo. We compute the pixel-to-cm ratio and perspective.</li>
              <li><b>Verify:</b> Enter the artwork size (e.g., 100 × 70 cm). The mockup snaps to the exact scale on your wall photo.</li>
            </ol>
...
function Pricing() {
  const tiers = [
    ...
    },
    {
      ...
      price: "€9 / mo",
      highlight: true,
      features: ["10 room presets", "Up to 50 artworks", "Up to 5 sizes per artwork", "Swatches + color picker", "Designer Mode enabled (width input)"],
      cta: "Choose Basic",
    },
    {
      name: "Designer Pro",
        ...
        <div className="relative overflow-hidden rounded-3xl border border-slate-200 bg-[radial-gradient(1200px_500px_at_50%_0%,color-mix(in_oklab,var(--accent),white_85%),white)] p-8 sm:p-12">
          ...
          <div className="mx-auto mt-8 max-w-3xl text-sm text-slate-700">
            ...
            <ol className="mt-2 list-decimal space-y-2 pl-5">
              <li>Create a placeholder container: <code className="rounded bg-slate-100 px-1">{"<div id=\"roomvibe-root\"></div>"}</code></li>
              <li>Add the script (UMD) or install the React package (ESM).</li>
              <li>Configure props: <code className="rounded bg-slate-100 px-1">mode</code>, <code className="rounded bg-slate-100 px-1">collection</code>, <code className="rounded bg-slate-100 px-1">oneClickBuy</code>.</li>
              <li>Publish and test. Open DevTools to watch <code className="rounded bg-slate-100 px-1">onEvent</code> logs.</li>
            </ol>

            <h4 className="mt-4 text-sm font-semibold">Shopify (quick note)</h4>
            <p className="mt-1">Online Store → Themes → Edit code → add the container + UMD script in the desired template/section. For app-block integration, we’ll ship a block later.</p>
  ...
  return (
    <Container id="faq">
      <div className="mx-auto max-w-3xl text-center">
        ...
        <p className="mt-2 text-slate-600">Deep-dive answers about how RoomVibe works.</p>
      </div>
...
function SiteFooter() {
  ...
  function handleNewsletterSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setNlEmail("");
      setNlMsg("Thanks! You’re subscribed. We’ll send art tips and early room presets.");
    } catch {
  ...
  function handleContactSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setCStatus("Thanks! Your message is saved locally. We’ll reach you shortly.");
    } catch {
      ...
      <Container>
        <div className="grid gap-10 py-14 lg:grid-cols-3">
          ...
          <div>
            ...
            <p className="mt-2 text-sm/6 text-white/90">Have a question? Send us a message.</p>
            ...
            {cStatus && <div className="mt-2 text-xs text-white">{cStatus}</div>}
          </div>
        </div>
        ...
        <div className="flex items-center justify-between border-t border-white/20 py-6 text-xs text-white/80">
          ...
          <div className="flex items-center gap-4">
            <a href="#home" className="hover:underline underline-offset-2">Back to top</a>
          </div>
        </div>
      </Container>
    </footer>
    ...
    <div className="fixed inset-0 z-[60]">
      ...
      <div className="absolute left-1/2 top-1/2 w-[min(96vw,560px)] -translate-x-1/2 -translate-y-1/2 rounded-2xl border border-slate-200 bg-white p-5 shadow-xl">
        <div className="flex items-center justify-between">
          ...
          <button onClick={onClose} className="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-slate-200 hover:bg-slate-50">
