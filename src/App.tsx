    ...
    <div className="h-screen bg-[#1a1a1a] relative">
      ...
      {selectedArtwork && (
        ...
        >
          ...
          >
            {/* Image */}
            <div className="relative bg-gray-50 flex items-center justify-center" style={{ minHeight: '180px', maxHeight: '260px' }}>
              {selectedArtwork.artworkUrl && (
                <img 
            ...
            <div className="overflow-y-auto flex-1 px-6 pt-5 pb-2">
              <h2 className="text-xl font-bold text-[#1A2A38] leading-tight">
                {selectedArtwork.artworkTitle || 'Untitled'}
              </h2>
              {selectedArtwork.artistName && (
                <p className="text-sm text-gray-500 mt-0.5 mb-3">{selectedArtwork.artistName}</p>
              )}
              ...
              {selectedArtwork.priceAmount && (
                <p className="text-sm font-semibold text-[#264C61] mb-3">
                  {selectedArtwork.priceCurrency || 'EUR'} {Number(selectedArtwork.priceAmount).toLocaleString('en-US', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}
                </p>
              )}
              ...
              {selectedArtwork.story && (
                <div className="mt-1 mb-3">
                  <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1.5">Story Behind</p>
                  <p className="text-sm text-gray-600 leading-relaxed line-clamp-5 whitespace-pre-line">
                    {selectedArtwork.story}
                  </p>
                </div>
              )}
            ...
            {/* Buttons */}
            <div className="px-6 pb-6 pt-3 border-t border-gray-100 flex flex-col gap-2.5">
              {selectedArtwork.sourceArtworkId && (
                <a
                  href={`#/studio?artworkId=${selectedArtwork.sourceArtworkId}&from=${encodeURIComponent(window.location.hash || '#/')}`}
