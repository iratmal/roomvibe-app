...
export function DesignerArtLibrary({ onArtworkSelect, projects = [] }: DesignerArtLibraryProps) {
  ...
  const [showStudioModal, setShowStudioModal] = useState<LibraryArtwork | null>(null);
  const [newProjectTitle, setNewProjectTitle] = useState('');
  const [creatingProject, setCreatingProject] = useState(false);
  const [studioProjectId, setStudioProjectId] = useState<number | null>(null);
  ...
  const handleUseInStudio = (artwork: LibraryArtwork) => {
    setShowStudioModal(artwork);
    setSelectedArtwork(null);
    setStudioProjectId(null);
    setNewProjectTitle('');
  };

  const handleCreateProjectAndGoToStudio = async () => {
    if (!showStudioModal || !newProjectTitle.trim()) return;
    ...
    try {
      ...
      navigateToStudio(showStudioModal.id, data.project.id);
    } catch (err: any) {
      alert(err.message);
    } finally {
  ...
  const handleSelectProjectAndGoToStudio = () => {
    if (!showStudioModal || !studioProjectId) return;
    navigateToStudio(showStudioModal.id, studioProjectId);
  };

  const navigateToStudio = (artworkId: number, projectId?: number) => {
    const params = new URLSearchParams();
    params.set('artworkId', String(artworkId));
    if (projectId) {
  ...
  return (
    <div className="space-y-6">
          ...
          >
            ...
            <div className="p-6">
              ...
              <div className="space-y-4">
                <button
                  onClick={() => handleUseInStudio(selectedArtwork)}
                  className="w-full px-6 py-3 bg-[#D8B46A] text-white rounded-xl hover:bg-[#C9A55B] transition-colors font-medium"
                >
                  Use in Studio
                </button>
      ...
      {showStudioModal && (
        <div 
          className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4"
          onClick={() => setShowStudioModal(null)}
        >
          <div 
          ...
          >
            <div className="p-6 border-b border-slate-100">
              <div className="flex items-center justify-between">
                <h3 className="text-lg font-semibold text-[#264C61]">Use in Studio</h3>
                <button
                  onClick={() => setShowStudioModal(null)}
                  className="p-2 hover:bg-slate-100 rounded-lg transition-colors"
                >
            ...
            <div className="p-6">
              <div className="flex items-center gap-3 mb-6 p-3 bg-slate-50 rounded-xl">
                <img
                  src={showStudioModal.imageUrl}
                  alt={showStudioModal.title}
                  className="w-16 h-16 rounded-lg object-cover"
                />
                <div>
                  <p className="font-medium text-slate-800">{showStudioModal.title}</p>
                  <p className="text-sm text-slate-500">by {showStudioModal.artist.name}</p>
                </div>
              </div>
              ...
              {projects.length > 0 ? (
                <div className="space-y-4">
                  <div>
                    <label className="block text-sm font-medium text-slate-700 mb-2">Select a project</label>
                    <select
                      value={studioProjectId || ''}
                      onChange={(e) => setStudioProjectId(Number(e.target.value) || null)}
                      className="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#264C61] bg-slate-50"
                    >
                  ...
                  <button
                    onClick={handleSelectProjectAndGoToStudio}
                    disabled={!studioProjectId}
                    className="w-full px-6 py-3 bg-[#D8B46A] text-white rounded-xl hover:bg-[#C9A55B] transition-colors font-medium disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    Open in Studio
                  </button>
                  ...
                  <div className="relative my-4">
                    ...
                    <div className="relative flex justify-center text-sm">
                      <span className="px-2 bg-white text-slate-500">or create a new project</span>
                    </div>
                  </div>
                  ...
                  <div>
                    <input
                      ...
                      className="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#264C61] bg-slate-50 mb-2"
                      placeholder="New project name..."
                    />
                    <button
                      onClick={handleCreateProjectAndGoToStudio}
                      disabled={creatingProject || !newProjectTitle.trim()}
                      className="w-full px-6 py-3 border border-[#264C61] text-[#264C61] rounded-xl hover:bg-slate-50 transition-colors font-medium disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      {creatingProject ? 'Creating...' : 'Create Project & Open Studio'}
                    </button>
                  </div>
                </div>
              ) : (
                <div className="space-y-4">
                  <p className="text-sm text-slate-600 text-center">
                    Create a project to organize your work and use this artwork in Studio.
                  </p>
                  <div>
                    <input
                      type="text"
