...
export function GallerySentMessages() {
  ...
  const fetchMessages = async () => {
    try {
      const response = await fetch(`${API_URL}/api/gallery/sent-messages`, {
        credentials: 'include'
      });
      ...
      {loading ? (
        ...
        <div className="text-center py-16 px-8 bg-white rounded-2xl border border-slate-100">
          ...
          <p className="text-slate-500 text-sm">
            When you contact artists about exhibitions or collaborations, your messages will appear here
          </p>
        </div>
      ) : (
