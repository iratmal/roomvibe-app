...
router.put('/artworks/:id', authenticateToken, upload.single('image'), async (req: any, res) => {
  try {
    ...
    const result = await query(
      ...
      [title, imageUrl, storageKey, parseFloat(width), parseFloat(height), unit, priceAmount ? parseFloat(priceAmount) : null, currency, (buyUrl && buyUrl !== 'null') ? buyUrl : null, artworkOrientation, styleTagsJson, dominantColorsJson, artworkMedium, artworkAvailability, artworkVisibleToDesigners, artworkVisibleToGalleries, variantsJson, artworkCardImageId, artworkCleanImageId, artworkStory, artworkDescription, artworkId]
    );
