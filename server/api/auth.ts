...
router.post('/register', async (req: Request, res: Response) => {
  try {
    ...
    const validRoles = ['user', 'artist', 'designer', 'gallery', 'allaccess'];
    ...
    // Set entitlements based on selected role
    const artistAccess = role === 'artist' || role === 'allaccess';
    const designerAccess = role === 'designer' || role === 'allaccess';
    const galleryAccess = role === 'gallery' || role === 'allaccess';
    const effectiveRole = role === 'allaccess' ? 'user' : role;
    ...
    const result = await query(
      ...
      [email.toLowerCase(), passwordHash, effectiveRole, confirmationToken, true, now, now, tosVersion, marketingOptIn, marketingOptIn ? now : null, artistAccess, designerAccess, galleryAccess]
    );
