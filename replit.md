...
- **Studio Mode**:
    ...
    - **Dashboard-Studio Connection (Dec 2024)**: Studio fetches logged-in user's artworks via `/api/artist/mine` endpoint (role-agnostic). Artwork selector shows "My Artworks" optgroup first, then "Sample Artworks". Auto-selects first user artwork when available. Shows fallback message with Dashboard link when user has no uploaded artworks. Works for all account types (User, Artist, Designer, Gallery).
    ...
    - **AI Suggested Rooms (Updated Dec 2024)**: Smart room suggestions based on artwork tags, title keywords, and deterministic variation. Located in `src/lib/ai/roomSuggest.ts`. Shows "AI Suggested Rooms" section at top of room list (3 rooms, 3-column grid with gold ring accent). Uses heuristic scoring matching artwork tags to room categories (bedroom: calm/soft/romantic, livingroom: family/warm/neutral, kids: playful/colorful, cafe/kitchen: coffee/food, bathroom: minimal/clean/spa). Falls back to title keyword extraction when tags are empty. Hash-based tie-breaking ensures different artworks always produce different suggestions while remaining deterministic. Recalculates dynamically when user selects different artwork. Only visible when category filter is "All".
