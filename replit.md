...
- **Studio Mode**:
    ...
    - **AI Suggested Rooms (Dec 2024)**: Smart room suggestions based on artwork tags. Located in `src/lib/ai/roomSuggest.ts`. Shows "AI Suggested Rooms" section at top of room list (3 rooms, 3-column grid with gold ring accent). Uses heuristic scoring matching artwork tags to room categories (bedroom: calm/soft/romantic, livingroom: family/warm/neutral, kids: playful/colorful, cafe/kitchen: coffee/food, bathroom: minimal/clean/spa). Falls back to default rooms when artwork has no tags. Only visible when category filter is "All".
